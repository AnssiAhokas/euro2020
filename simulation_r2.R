...
#r4_data <- read.csv(paste0(here::here(), "/data/r4.csv"), sep = ",")

simulate_r2 <- function(teams_data, matches_data, r4_data, randomness){
  ...
  data <- matches_data %>% 
    ...
    left_join(x = ., y = select(r4_data, role, team), by = c("role" = "role")) %>% 
    select(-role) %>% 
  ...
  data <- data %>% 
    ...
    mutate(role = ifelse(winner == team, 
           ...
           position = ifelse(winner == team, "1", "2")) %>% 
    arrange(match_id) %>% 
    select(team, role, position)
  
  data <- rbind(r4_data %>% filter(is.na(position) == FALSE) %>% select(-X), data) %>% 
    arrange(position)
  
  ## Write a CSV file for the simulations
  write.csv(data, paste0(here::here(), "/data/r2.csv"))
  
  return(data)
  
}

#final_data_r2 <- simulate_r2(teams_data = teams_data, 
 #                            matches_data = matches_data, 
  #                           r4_data = r4_data, 
   #                          randomness = 1)